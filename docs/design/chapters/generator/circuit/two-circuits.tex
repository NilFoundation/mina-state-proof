...
\begin{enumerate}
	\item for each $\pi_i$:
        ...
        \end{enumerate}
        \item $\texttt{batch\_verify\_scalar\_field}(\mathcal{P}_0, \dots, \mathcal{P}_{\texttt{batch\_size}})$
\end{enumerate}
\end{algorithm}
...
\begin{enumerate}
	\item for each $\pi_i$:
        \begin{enumerate}
            ...
            \item $\mathcal{P}_i = \{H_{\mathbb{F}_q}, \zeta, v, u, \textbf{PE}, o_{\pi_i} \}$
        \end{enumerate}
	\item $\texttt{batch\_verify\_base\_field}(\mathcal{P}_0, \dots, \mathcal{P}_{\texttt{batch\_size}})$
\end{enumerate}
\end{algorithm}
