...
library redshift_verifier_poseidon_component {
    ...
    function parse_verify_proof_be(
        ...
        uint256[] memory assignments_ptrs = new uint256[](
            poseidon_component.WITNESS_ASSIGNMENTS_N
        );
        local_vars.tmp1 = 0;
        local_vars.offset =
            proof_map.eval_proof_witness_offset +
            ...
            for (
                ...
                require(
                    common_data.columns_rotations[i][j] ==
                        poseidon_component.get_rotation(i, j),
        ...
        gate_params.selector_evaluations_ptrs = new uint256[](
            poseidon_component.GATES_N
        );
        local_vars.offset =
            proof_map.eval_proof_selector_offset +
            basic_marshalling.LENGTH_OCTETS;
        for (uint256 i = 0; i < poseidon_component.GATES_N; i++) {
            gate_params.selector_evaluations_ptrs[i] = lpc_verifier
                .get_z_i_ptr_from_proof_be(blob, local_vars.offset, 0);
            local_vars.offset = lpc_verifier.skip_proof_be(
                blob,
                local_vars.offset
            );
        }
        local_vars.gate_argument = poseidon_component.evaluate_gates_be(
            assignments_ptrs,
            gate_params
        );
