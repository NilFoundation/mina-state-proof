...
from state_proof_params import init_placeholder_params, linked_mina_state_libs_names
...
def deploy(test_contract_name, test_contract_path):
    init_profiling()
    w3 = init_connection()
    ...
    bytecode = deploy_link_libs(w3, compiled, bytecode, linked_mina_state_libs_names)
    ...
    return (w3, test_contract_inst)

def state_proof(w3, test_contract_inst, ledger_hash):
    params = init_placeholder_params(base_path + '/test/data/generated_base_eval10_step1_scalar_eval15_step1.data')
    
    # verify state without storing it
    print("State verification without storing it: ")
    run_tx_hash = test_contract_inst.functions.verify_ledger_state(ledger_hash,
    ...
    print_tx_info(w3, run_tx_receipt, 'State verification')

    # update last mina state
    print("Verify state and update the last ledger hash:")
    run_tx_hash = test_contract_inst.functions.update_ledger_state(ledger_hash,
        params['proof'], params['init_params'], params['columns_rotations']).transact()
    run_tx_receipt = w3.eth.wait_for_transaction_receipt(run_tx_hash)
    print_tx_info(w3, run_tx_receipt, 'State update')
    ...
    if "--nolog" not in sys.argv:
        if hasattr(test_contract_inst.events, "gas_usage_emit"):
            ...
            logfilename = base_path + "/test/" + logfilename
...
def account_proof(w3, test_contract_inst, account_hash):
    params = init_placeholder_params(base_path + '/test/data/generated_base_eval10_step1_scalar_eval15_step1.data')
    
    # verify state without storing it
    print("State verification without storing it: ")
    run_tx_hash = test_contract_inst.functions.verify_ledger_state(account_hash,
        params['proof'], params['init_params'], params['columns_rotations']).transact()
    run_tx_receipt = w3.eth.wait_for_transaction_receipt(run_tx_hash)
    print_tx_info(w3, run_tx_receipt, 'State verification')
    ...
    # update last mina state
    print("Verify state and update the last ledger hash:")
    run_tx_hash = test_contract_inst.functions.update_ledger_state(account_hash,
        params['proof'], params['init_params'], params['columns_rotations']).transact()
    run_tx_receipt = w3.eth.wait_for_transaction_receipt(run_tx_hash)
    print_tx_info(w3, run_tx_receipt, 'State update')
...
if __name__ == '__main__':
    test_contract_name = 'MinaBridgeEndpoint'
    test_contract_path = '../contracts/mina_bridge_endpoint.sol'
    ledger_hash = 'jxYxn25DLi58412b2o44PXRDLzwLK3JLx36kcpu71DxAYZqwa28'
    # prepare contract
    w3, test_contract_inst = deploy(test_contract_name, test_contract_path)
    # update ledger state
    state_proof(w3, test_contract_inst, ledger_hash)
    # send account state
    account_proof(w3, test_contract_inst, ledger_hash)