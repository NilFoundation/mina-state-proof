...
#include <iomanip>
    ...
    !(defined(CI_SUPPRESS_KNOWN_ISSUES) && defined(__GNUC__) && defined(_WIN32))
...
nil::crypto3::multiprecision::cpp_dec_float_50 JEL2(nil::crypto3::multiprecision::cpp_dec_float_50 v,
                                                    nil::crypto3::multiprecision::cpp_dec_float_50 z) {
    return boost::math::tgamma(v + 1) * boost::math::cyl_bessel_j(v, z) / nil::crypto3::multiprecision::pow(z / 2, v);
}

/*`
The implementation is almost the same as before, but with one key difference - we can no longer call
`std::pow`, instead we must call the version inside the `nil::crypto3::multiprecision` namespace.  In point of
...
to `nil::crypto3::multiprecision::pow` (when the argument is a multiprecision type).  We do that by
...
Unfortunately there is a problem with this version: if we were to call it like this:
   ...
   nil::crypto3::multiprecision::cpp_dec_float_50 v(2), z(0.5);
   JEL3(v + 0.5, z);
...
typename boost::math::tools::promote_args<Float1, Float2>::type JEL4(Float1 v, Float2 z) {
    using std::pow;
    return boost::math::tgamma(v + 1) * boost::math::cyl_bessel_j(v, z) / pow(z / 2, v);
}
...
template<typename value_type, typename function_type>
inline value_type integral(const value_type a, const value_type b, const value_type tol, function_type func) {
    unsigned n = 1U;
...
public:
    cyl_bessel_j_integral_rep(const unsigned N, const value_type& X) : n(N), x(X) {
    }
...
typedef nil::crypto3::multiprecision::number<cpp_dec_float<64>> mp_type;
...
mp_type mysin(const mp_type& x) {
    ...
    static const std::array<mp_type, 32U> coefs = {{
        ...
        mp_type(
            "-0.64596409750624625365575656389794573337969351178927307696134454382929989411386887578263960484"),    // ^3
        mp_type(
            "+0.07969262624616704512050554949047802252091164235106119545663865720995702920146198554317279"),       // ^5
    ...
    //
    const mp_type sum =
        (((((((((((((((((((((((((((((((+coefs[31U] * x2 + coefs[30U]) * x2 + coefs[29U]) * x2 + coefs[28U]) * x2 +
                                    coefs[27U]) *
                                       x2 +
                                   coefs[26U]) *
                                      x2 +
...
int main() {
    ...
    using nil::crypto3::multiprecision::cpp_dec_float_50;
    ...
    // 4.75292
    std::cout << std::setprecision(std::numeric_limits<float>::digits10) << a_f << std::endl;

    // 4.752915525616
    std::cout << std::setprecision(std::numeric_limits<double>::digits10) << a_d << std::endl;

    // 4.7529155256159981904701331745635599135018975843146
    std::cout << std::setprecision(std::numeric_limits<cpp_dec_float_50>::digits10) << a_mp << std::endl;
    /*=}*/
    ...
    //
    const float d_f = derivative(pi<float>() / 3, 0.01F, static_cast<float (*)(float)>(std::sin));

    const double d_d = derivative(pi<double>() / 3, 0.001, static_cast<double (*)(double)>(std::sin));
    //
    ...
    //
    const cpp_dec_float_50 d_mp = derivative(cpp_dec_float_50(pi<cpp_dec_float_50>() / 3),
                                             cpp_dec_float_50(1.0E-9),
                                             [](const cpp_dec_float_50& x) -> cpp_dec_float_50 { return sin(x); });

    // 5.000029e-001
    std::cout << std::setprecision(std::numeric_limits<float>::digits10) << d_f << std::endl;

    // 4.999999999998876e-001
    std::cout << std::setprecision(std::numeric_limits<double>::digits10) << d_d << std::endl;

    // 4.99999999999999999999999999999999999999999999999999e-01
    std::cout << std::setprecision(std::numeric_limits<cpp_dec_float_50>::digits10) << d_mp << std::endl;
    //=}
    ...
    */
    ...
    cpp_dec_float_50 gd =
        derivative(cpp_dec_float_50(2), cpp_dec_float_50(1.0E-9), [](const cpp_dec_float_50& x) -> cpp_dec_float_50 {
            return boost::math::gamma_p(2, x);
        });
    // 2.70670566473225383787998989944968806815263091819151e-01
    std::cout << std::setprecision(std::numeric_limits<cpp_dec_float_50>::digits10) << gd << std::endl;
    ...
    {*/
    using boost::math::constants::pi;
    typedef nil::crypto3::multiprecision::cpp_dec_float_50 mp_type;

    const float j2_f = integral(0.0F, pi<float>(), 0.01F, cyl_bessel_j_integral_rep<float>(2U, 1.23F)) / pi<float>();

    const double j2_d = integral(0.0, pi<double>(), 0.0001, cyl_bessel_j_integral_rep<double>(2U, 1.23)) / pi<double>();

    const mp_type j2_mp =
        integral(
            mp_type(0), pi<mp_type>(), mp_type(1.0E-20), cyl_bessel_j_integral_rep<mp_type>(2U, mp_type(123) / 100)) /
        pi<mp_type>();

    // 0.166369
    std::cout << std::setprecision(std::numeric_limits<float>::digits10) << j2_f << std::endl;

    // 0.166369383786814
    std::cout << std::setprecision(std::numeric_limits<double>::digits10) << j2_d << std::endl;

    // 0.16636938378681407351267852431513159437103348245333
    std::cout << std::setprecision(std::numeric_limits<mp_type>::digits10) << j2_mp << std::endl;
    ...
    std::cout << nil::crypto3::multiprecision::sin(boost::math::constants::pi<::mp_type>() / 4) << std::endl;
