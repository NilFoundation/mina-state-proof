...
typedef nil::crypto3::multiprecision::cpp_dec_float_50 real_type;
#elif defined(TEST_MPFR_50)
#include <nil/crypto3/multiprecision/mpfr.hpp>
typedef nil::crypto3::multiprecision::mpfr_float_50 real_type;
#elif defined(TEST_MPF_50)
#include <nil/crypto3/multiprecision/gmp.hpp>
typedef nil::crypto3::multiprecision::mpf_float_50 real_type;
...
std::ostream& operator<<(std::ostream& os, const __float128& f) {
    return os << nil::crypto3::multiprecision::float128(f);
}
...
#include <nil/crypto3/multiprecision/float128.hpp>
typedef nil::crypto3::multiprecision::float128 real_type;
#elif defined(TEST_CPP_BIN_FLOAT_QUAD)
#include <nil/crypto3/multiprecision/cpp_bin_float.hpp>
typedef nil::crypto3::multiprecision::cpp_bin_float_quad real_type;
#elif defined(TEST_CPP_BIN_FLOAT_OCT)
#include <nil/crypto3/multiprecision/cpp_bin_float.hpp>
typedef nil::crypto3::multiprecision::cpp_bin_float_oct real_type;
#else
...
extern "C" {
#include "f2c.h"
integer s_wsfe(cilist*), e_wsfe(void), do_fio(integer*, char*, ftnlen), s_wsle(cilist*),
...
int dgefa_(real_type*, integer*, integer*, integer*, integer*),
    dgesl_(real_type*, integer*, integer*, integer*, real_type*, integer*);
...
extern "C" int MAIN__() {
    ...
    */
    std::cout << std::setw(12) << std::setprecision(5) << residn << " " << resid << " " << eps << " " << x[0] << " "
              << x[n - 1] << std::endl;
    e_wsfe();
...
/* Subroutine */ int matgen_(real_type* a, integer* lda, integer* n, real_type* b, real_type* norma) {
    /* System generated locals */
...
/* Subroutine */ int daxpy_(integer* n, real_type* da, real_type* dx, integer* incx, real_type* dy, integer* incy) {
    /* System generated locals */
    integer i__1;
...
real_type ddot_(integer* n, real_type* dx, integer* incx, real_type* dy, integer* incy) {
    /* System generated locals */
    integer i__1;
    real_type ret_val;
...
/* Subroutine */ int dscal_(integer* n, real_type* da, real_type* dx, integer* incx) {
    /* System generated locals */
    integer i__1, i__2;
...
/* Subroutine */ int mm_(real_type* a, integer* lda, integer* n1, integer* n3, real_type* b, integer* ldb, integer* n2,
                         real_type* c__, integer* ldc) {
    /* System generated locals */
...
/* Subroutine */ int dmxpy_(integer* n1, real_type* y, integer* n2, integer* ldm, real_type* x, real_type* m) {
    ...
    if (j >= 4) {
        i__1 = *n1;
        for (i__ = 1; i__ <= i__1; ++i__) {
            y[i__] = y[i__] + x[j - 3] * m[i__ + (j - 3) * m_dim1] + x[j - 2] * m[i__ + (j - 2) * m_dim1] +
                     x[j - 1] * m[i__ + (j - 1) * m_dim1] + x[j] * m[i__ + j * m_dim1];
            /* L30: */
        }
    }
    ...
    if (j >= 8) {
        i__1 = *n1;
        for (i__ = 1; i__ <= i__1; ++i__) {
            y[i__] = y[i__] + x[j - 7] * m[i__ + (j - 7) * m_dim1] + x[j - 6] * m[i__ + (j - 6) * m_dim1] +
                     x[j - 5] * m[i__ + (j - 5) * m_dim1] + x[j - 4] * m[i__ + (j - 4) * m_dim1] +
                     x[j - 3] * m[i__ + (j - 3) * m_dim1] + x[j - 2] * m[i__ + (j - 2) * m_dim1] +
                     x[j - 1] * m[i__ + (j - 1) * m_dim1] + x[j] * m[i__ + j * m_dim1];
            /* L40: */
        }
    }
    ...
    for (j = jmin; j <= i__1; j += 16) {
        i__2 = *n1;
        for (i__ = 1; i__ <= i__2; ++i__) {
            y[i__] = y[i__] + x[j - 15] * m[i__ + (j - 15) * m_dim1] + x[j - 14] * m[i__ + (j - 14) * m_dim1] +
                     x[j - 13] * m[i__ + (j - 13) * m_dim1] + x[j - 12] * m[i__ + (j - 12) * m_dim1] +
                     x[j - 11] * m[i__ + (j - 11) * m_dim1] + x[j - 10] * m[i__ + (j - 10) * m_dim1] +
                     x[j - 9] * m[i__ + (j - 9) * m_dim1] + x[j - 8] * m[i__ + (j - 8) * m_dim1] +
                     x[j - 7] * m[i__ + (j - 7) * m_dim1] + x[j - 6] * m[i__ + (j - 6) * m_dim1] +
                     x[j - 5] * m[i__ + (j - 5) * m_dim1] + x[j - 4] * m[i__ + (j - 4) * m_dim1] +
                     x[j - 3] * m[i__ + (j - 3) * m_dim1] + x[j - 2] * m[i__ + (j - 2) * m_dim1] +
                     x[j - 1] * m[i__ + (j - 1) * m_dim1] + x[j] * m[i__ + j * m_dim1];
            /* L50: */
...
real_type ran_(integer* iseed) {
    ...
    ret_val = ((real_type)it1 +
               ((real_type)it2 + ((real_type)it3 + (real_type)it4 * 2.44140625e-4) * 2.44140625e-4) * 2.44140625e-4) *
              2.44140625e-4;
    return ret_val;
...
nil::crypto3::multiprecision::ef::cpp_dec_float_50:
~~~~~~~~~~~~~~~~~~~~~~~~~
